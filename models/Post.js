...
export class Post {
      ...
      return rows.map((row, index) => {
        try {
          ...
          const safeString = (value, fallback = '') => {
            ...
            if (Buffer.isBuffer(value)) {
              try {
                ...
                console.warn('⚠️ Server: Failed to convert Buffer to string:', e.message)
                return fallback
              }
            }
          ...
          return {
            ...
            caption: safeString(row.caption || row.description || ''),
            rating: (() => {
  ...
  static async getById(id) {
    try {
      ...
      return {
        ...
        caption: Buffer.isBuffer(row.caption) ? row.caption.toString('utf8') : (row.caption || row.description || 'd'),
        images: Array.isArray(images) ? images : [],
        videos: Array.isArray(videos) ? videos : [],
        tags: Array.isArray(tags) ? tags : [],
        rating: avgRating,
  ...
  static async delete(id) {
    ...
    if (postIdStr) {
      identifiers.add(postIdStr)
    }

    if (/^\d+$/.test(postIdStr)) {
      identifiers.add(String(parseInt(postIdStr, 10)))
    }

    const extracted = postIdStr.match(/post-(\d+)/)
    if (extracted && extracted[1]) {
      identifiers.add(extracted[1])
    }
